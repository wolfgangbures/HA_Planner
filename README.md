...
This code was generated by GPT5.1-Codex.
...
- ✅ Add comments/notes to existing tasks
...
Use the `planner.add_task_comment` service to append notes to an existing task.
The comment is written to the task's **Notes** field in Planner (the `description` in Microsoft Graph).

#### Via Developer Tools → Services:
```yaml
service: planner.add_task_comment
data:
  task_id: "AAMkADk3..."
  comment: "Reviewed and approved on Monday."
```

By default the new text is **appended** to any existing notes (separated by a blank line).
Set `append: false` to **overwrite** the existing notes instead:

```yaml
service: planner.add_task_comment
data:
  task_id: "AAMkADk3..."
  comment: "Replaces previous notes entirely."
  append: false
```
